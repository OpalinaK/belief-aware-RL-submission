...
This report studies several preliminary experiments that set up the belief-aware RL
question.
...
Second, we run complementary baselines using the same Gym interface under the
\textbf{RMSC04} background calibration (\texttt{test\_rmsc04.py}): four
\textbf{investor-role} heuristics---\texttt{NOISE}, \texttt{MARKET\_MAKER},
\texttt{MOMENTUM}, and \texttt{VALUE}---that read the RL observation tensor.
Results and figures live in \texttt{plots\_rmsc04/}. These policies are named
similarly to background agent types but are \emph{simplified mappings} from Gym
states to \{buy, hold, sell\}; they answer how much edge each \emph{narrative}
has for the learner, not how the kernel's full agents behave.

Third, we run \textbf{native ABIDES} simulations without the Gym investor
wrapper (\texttt{baselines.py}): hundreds of stochastic seeds with the kernel's
real \texttt{NoiseAgent}, \texttt{ValueAgent}, \texttt{MomentumAgent}, and
\texttt{AdaptiveMarketMakerAgent} implementations (real schedules, oracle access
where coded, ladders, etc.). Output is summarized in JSON and plots under
\texttt{sim\_plots/}. Ranking here can diverge sharply from Gym heuristics; see
Section~\ref{sec:rm04-native-extra-baselines}.

Fourth, we run one-at-a-time parameter sweeps over the background market and
...
\begin{enumerate}
    ...
    simulated regime. Where helpful, report Gym metrics (episode-level) and---for
    narrative checks---native pooled metrics without mixing Sharpe definitions.
\end{enumerate}
...
sections show which policies work under the default six-strategy calibration,
then contrasts RMSC04 Gym four-strategy runs and native kernel aggregates
(Section~\ref{sec:rm04-native-extra-baselines}), before the parameter sweeps.
Section~\ref{sec:design-directions} collects implementation hooks for the learner.
The conclusion translates these findings into the next belief-aware RL plan.
...
\section{Metrics and Sweep Parameters}

Before looking at the results, it helps to separate two kinds of quantities in
this report. The \textbf{metrics} are what we measure after an episode finishes.
The \textbf{sweep parameters} are the knobs we change in the simulator to create
...
\midrule
Final P\&L &
The final mark-to-market value minus the starting cash account. Positive P\&L
means the strategy ended the episode with more value than it started with. All
P\&L values in this report are measured in cents. \\
Total reward &
The sum of per-step rewards over one episode. In this environment, reward is
based on changes in mark-to-market value, so it should tell a similar story to
P\&L. \\
Sharpe ratio &
A simple risk-adjusted score: mean final P\&L divided by the standard deviation
of final P\&L across episodes. A higher Sharpe means the strategy earns more
per unit of variability. \\
Win rate &
The fraction of episodes where final P\&L is non-negative. This is useful
because a strategy can have a high average P\&L but still lose often if its
returns are very skewed. \\
Maximum drawdown &
The largest peak-to-trough loss during an episode, measured as a fraction of
peak mark-to-market value. Lower drawdown means the strategy is safer during
the day. \\
Transaction-cost proxy &
The sum of roughly half the spread paid on non-hold actions. It is not an exact
exchange fee; it is a simple measure of how much active trading pays to cross
the bid-ask spread. \\
Episode length &
How many environment steps the episode lasts. Short episodes would indicate
early termination from the drawdown kill switch; full-length episodes mean the
strategy survived the whole simulated day. \\
\bottomrule
\end{tabular}
\caption{Metrics used to compare baseline strategies and sweep results.}
\label{tab:metric-glossary}
\end{table}

\subsection{Sweep Parameters}

The sweep changes one parameter at a time. Each row below explains what that
parameter controls and why it matters for belief-aware trading.
...
The number of value agents in the background market. More value agents should
create stronger fundamental anchoring and make mean reversion more plausible. \\
\texttt{lambda\_a} &
The arrival rate of value-agent trading opportunities. Higher values mean value
agents show up more often, so the market should react more quickly when price
deviates from fundamental value. \\
\texttt{kappa} &
The mean-reversion strength assumed by value agents in their belief model.
Larger values make value agents believe deviations from fundamental value close
more quickly. \\
\texttt{kappa\_oracle} &
The true mean-reversion strength of the underlying fundamental-value process.
This changes the actual environment dynamics, not just what value agents
believe. \\
\texttt{fund\_vol} &
The volatility of the fundamental-value process. Higher volatility creates
larger price opportunities but also more risk. \\
\texttt{num\_momentum\_agents} &
The number of trend-following agents. More momentum agents should increase
trend persistence and make momentum-style trading more attractive. \\
\texttt{num\_noise\_agents} &
The number of random order-flow agents. Noise agents add volume and price
jitter; they can also affect book depth because market makers size orders based
on recent volume. \\
\texttt{mm\_wake\_up\_freq} &
How often adaptive market makers refresh their quotes. Slower refresh can leave
stale quotes in the book; faster refresh can remove stale-quote opportunities. \\
\texttt{mm\_spread\_alpha} &
How quickly the market-maker spread estimate adapts to recent market
conditions. This affects the spread cost paid by active strategies. \\
\texttt{mm\_window\_size} &
The market-maker lookback window for estimating spread and liquidity. This
changes how stable or reactive the quoted market is. \\
\texttt{order\_fixed\_size} &
The number of shares the daily investor buys or sells per market order. Larger
orders increase exposure but can also increase slippage and spread cost. \\
\texttt{timestep\_duration} &
How much simulated time passes between the daily investor's decisions. Shorter
timesteps allow more frequent reactions; longer timesteps produce coarser
observations and fewer trading opportunities. \\
\bottomrule
\end{tabular}
\caption{Parameters varied in the one-at-a-time sweep.}
\label{tab:sweep-parameter-glossary}
\end{table}
...
\section{RMSC04 Gym Investor Baselines Versus Native ABIDES Agent Baselines}
\label{sec:rm04-native-extra-baselines}

This section contrasts two empirical tracks that reuse the phrase ``momentum'' or
``value'' but evaluate \emph{different objects}. Confusing them would overstate how
much the Gym benchmarks say about ABIDES itself.

\subsection{What differs between tracks?}
...
Track & Definition \\
\midrule
\textbf{RMSC04 Gym} (\texttt{plots\_rmsc04/}) &
\texttt{markets-daily\_investor-v0}, \texttt{background\_config}=\texttt{rmsc04},
optional \texttt{background\_config\_extra\_kvargs}. One controlled investor
\texttt{buy}, \texttt{hold}, or \texttt{sell} at fixed timestep granularity; reward and early-stop
behavior follow the Gym spec. Policies are heuristic functions of the RL
observation (holdings, imbalance, spread, direction feature, padded returns).
\\
\textbf{Native simulation} (\texttt{sim\_plots/}) &
No Gym investor. The simulator runs bundled ABIDES agents with kernel-level
ordering, wakes, noise models where applicable (\texttt{baselines.py}). Metrics are
\textbf{aggregated over every instantiated agent} across seeds (not one episode per
strategy). Episode length plots in Gym are trivially full-length; native plots use
\textbf{active trading window} (minutes from first to last fill).
\\
\textbf{AAPL-calibrated six policies} (\texttt{plots/}, earlier section) &
Same Gym family as elsewhere but a different heuristic set (\texttt{MR},
\texttt{MR\_INV}, etc.). Not the same code paths as \texttt{test\_rmsc04.py}.
\\
\bottomrule
\end{tabular}
\caption{Three baseline tracks and what each one actually measures.}
\label{tab:baseline-tracks}
\end{table}

\subsection{RMSC04 Gym four-strategy summary (50 episodes per policy)}

Table~\ref{tab:rmsc04-gym-summary} summarizes \texttt{baseline\_results\_rmsc04.json}.
Sharpe is mean final P\&L divided by standard deviation across episodes---the same
episode-level convention as Table~\ref{tab:baseline-results}.
...
\begin{tabular}{lrrrr}
\toprule
Strategy & Mean P\&L (¢) & Sharpe & Win rate & Narrative \\
\midrule
\texttt{MOMENTUM}   & +19{,}948 & +0.93 & 86\% &
Trend-style rule on RL features; best among these four Gym heuristics. \\
\texttt{NOISE}      &  ~-8{,}287 & -0.31 & 36\% &
Biased random action mix; negative mean, high variance. \\
\texttt{VALUE}      & -12{,}584 & -0.67 & 22\% &
Mean-reversion-style rule on \texttt{obs[3]}; \emph{not} the kernel
\texttt{ValueAgent}. \\
\texttt{MARKET\_MAKER} & -32{,}718 & -2.51 & 2\% &
Inventory-style rule; worst mean P\&L in this bundle. \\
\bottomrule
\end{tabular}
\caption{RMSC04 Gym investor-style baselines from \texttt{test\_rmsc04.py}
(50 seeds, default timestep and order size in that script).}
\label{tab:rmsc04-gym-summary}
\end{table}
...
\begin{subfigure}{0.48\linewidth}
\centering
\includegraphics[width=\linewidth]{../../../plots_rmsc04/1_m2m_pnl_distribution.png}
\caption{P\&L distribution}
\end{subfigure}
\hfill
\begin{subfigure}{0.48\linewidth}
\centering
\includegraphics[width=\linewidth]{../../../plots_rmsc04/6_max_drawdown_cdf.png}
\caption{Max drawdown CDF}
\end{subfigure}
\caption{RMSC04 Gym baselines (\texttt{plots\_rmsc04/}). Interpret together with
Table~\ref{tab:rmsc04-gym-summary}.}
\label{fig:rmsc04-gym-plots-sample}
\end{figure}

\subsection{Native ABIDES agent-level summary}

Table~\ref{tab:native-sim-summary} summarizes \texttt{sim\_baseline\_results.json},
pooling many agents across many seeds (\texttt{baselines.py}). Sharpe is
\textbf{cross-sectional}: mean agent P\&L divided by standard deviation of P\&L over
\textbf{agents} (not episodic variance for one learner). Figures are analogous to the
six-plot Gym suite except plot~3 tracks active-fill windows.
...
\begin{tabular}{lrrrr}
\toprule
Type & Approx.\ $n$ & Mean P\&L (¢) & Sharpe$^\dagger$ & Win rate \\
\midrule
\texttt{VALUE}        & 5{,}100   & +212{,}000 & +0.72  & 82\% \\
\texttt{NOISE}        & 50{,}000  &       -20 & $\approx$0 & 41\% \\
\texttt{MARKET\_MAKER}& 100       & -617{,}000 & -0.24  & 29\% \\
\texttt{MOMENTUM}     &   600    & $-1.7\times10^6$ & -1.0 & 13\% \\
\bottomrule
\end{tabular}\\[0.35em]
\footnotesize{$^\dagger$Cross-sectional Sharpe across agents; not comparable by
numeric magnitude to episode-level Sharpes above.}
\caption{Representative aggregates from native \texttt{baselines.py} paired with RMSC04
parameters (counts reflect 50 stochastic seeds \(\times\) population per simulation).
Exact totals live in JSON alongside the plots.}
\label{tab:native-sim-summary}
\end{table}
...
\begin{subfigure}{0.48\linewidth}
\centering
\includegraphics[width=\linewidth]{../../../sim_plots/1_m2m_pnl_distribution.png}
\caption{P\&L (native agents)}
\end{subfigure}
\hfill
\begin{subfigure}{0.48\linewidth}
\centering
\includegraphics[width=\linewidth]{../../../sim_plots/4_sharpe_ratio.png}
\caption{Cross-sectional Sharpe by type}
\end{subfigure}
\caption{Native ABIDES baselines (\texttt{sim\_plots/}). KERNEL
\texttt{VALUE} dominates; KERNEL \texttt{MOMENTUM} loses heavily---the opposite
ranking flavor from heuristic Gym momentum on the RL observation.}
\label{fig:native-sim-plots-sample}
\end{figure}

\subsection{Caveats when talking to the belief-aware roadmap}

\textbf{Ranking is not transferable across tracks.} A momentum-friendly Gym benchmark
does not imply that the kernel \texttt{MomentumAgent} pool is profitable, or that
either object is optimal---only that the \emph{tested heuristic} interacts favorably with
(friction-filled) RL observations under that calibration.

\textbf{Prefer careful language.} Safer wording: \emph{this fixed rule earns higher
average return than our fixed mean-reversion rule under the stated settings}. Avoid ``optimal policy''
unless referring to an explicit optimization result.

\textbf{Sharpe definitions differ} between episodic summaries (tables in the main
baseline section and Table~\ref{tab:rmsc04-gym-summary}) and pooled-agent native
statistics (footnote in Table~\ref{tab:native-sim-summary}).

\textbf{For agent design}, treat Gym baselines as \textbf{orthogonal probes}: (i)
which hand splits of observable features look profitable in-friction today, and (ii)
how far native agents deviate once full microstructure and information sets are restored.
Both inform features and regimes worth modeling in \(b_t\), but neither alone defines
target behavior for \(\pi(o_t,b_t)\).

\subsection{Design directions for a belief-aware daily investor}
\label{sec:design-directions}

These are implementation-oriented consequences of the multi-track evidence above.

\begin{itemize}
    \item \textbf{Observation augmentation.} Keep the default Gym tensor, but add
    session context (e.g.\ time since open, fraction of day remaining) and stable
    scaling of spread, returns, and inventory so policies transfer across seeds and
    background \texttt{extra\_kvargs}.

    \item \textbf{Regime without hand-labeled switches.} Use a single expressive policy
    or a shared trunk with gated ``trend'' vs ``fade'' heads so the network learns
    when imbalance and short returns support continuation versus when the direction
    feature supports reversion---mirroring the sweep story (value-heavy backgrounds
    favoring MR) without hard-coding MR vs momentum.

    \item \textbf{Train under distribution shift.} Randomize RMSC04 knobs (noise,
    value, momentum counts, \texttt{fund\_vol}, timestep, \texttt{done\_ratio}) so
    \(\pi(o_t)\) does not memorize one calibration; evaluate with held-out seeds and,
    when relevant, compare to native-only metrics for sanity.

    \item \textbf{Risk beyond mean return.} Drawdown CDFs differ sharply across
    policies; auxiliary penalties or distributional objectives (e.g.\ CVaR-style
    weighting) align training with the tail behavior already visualized in the plots.

    \item \textbf{Baselines as probes, not targets.} Match learning agents against
    Gym heuristics for the investor interface (Tables~\ref{tab:baseline-results}
    and~\ref{tab:rmsc04-gym-summary}) and cite native pools
    (Table~\ref{tab:native-sim-summary}) to stress-test narrative: good Gym P\&L does
    not certify optimality or equivalence to kernel agents.
\end{itemize}
...
\begin{enumerate}
    ...
    \item \textbf{Gym baselines do not dictate native rankings.}
    Section~\ref{sec:rm04-native-extra-baselines} documents a concrete inversion:
    heuristic \texttt{MOMENTUM} on RL observations ranks first among four Gym
    policies while kernel \texttt{MomentumAgent}s lose massively in pooled native
    runs. Conversely, Gym \texttt{VALUE} heuristic underperforms, while kernel
    \texttt{ValueAgent}s dominate native aggregates. Claims about ``momentum'' or
    ``value'' must name which implementation and evaluation track they refer to.
...
The next iteration should make the sweep results more signaling. Right now,
many sweep plots have large error bars that overlap zero and overlap each other.
Those plots are useful diagnostics, but they are not strong enough to support
precise claims about every parameter.

\begin{itemize}
    \item \textbf{Most noisy sweeps.} The least conclusive sweeps are
    \texttt{num\_momentum\_agents}, \texttt{kappa}, \texttt{kappa\_oracle},
    \texttt{fund\_vol}, \texttt{num\_noise\_agents}, the market-maker
    parameters, \texttt{order\_fixed\_size}, and \texttt{timestep\_duration}.
    Their error bars are wide and the strategy rankings are not stable across
    parameter values.

    \item \textbf{Most useful sweep so far.} The clearest signal is
    \texttt{num\_value\_agents}. As value-agent count increases,
    \texttt{MR} improves while \texttt{MOMENTUM} weakens. This directly
    supports the idea that hidden market composition changes which trading
    behavior is best.

    \item \textbf{Improve statistical signal.} Run more episodes per
    parameter-value-strategy cell, save the raw \texttt{sweep\_results.json},
    and plot confidence intervals rather than standard deviation bars.
    Confidence intervals answer the question ``how certain are we about the
    mean?'' more directly than standard deviations.

    \item \textbf{Use stronger regime definitions.} Instead of only changing
    one parameter slightly at a time, create explicit regimes: a
    momentum-heavy regime, a value-agent-heavy regime, a noise-heavy regime,
    and a mixed regime. This should produce cleaner differences for the belief
    module to learn.

    \item \textbf{Simplify the diagnostic plots.} For the next sweep, focus on
    \texttt{MOMENTUM}, \texttt{MR}, \texttt{MR\_INV}, and \texttt{HOLD}. The
    full six-strategy plots are crowded, while the belief-aware RL question is
    mainly about whether the market favors momentum or mean reversion.

    \item \textbf{Plot differences from default.} In addition to raw P\&L,
    plot each strategy's change in mean P\&L relative to the default parameter
    value. This will make it easier to see whether a parameter actually moves
    the market regime.

    \item \textbf{Keep the three baseline tracks synchronized.} Whenever
    \texttt{background\_config\_extra\_kvargs} or RMSC04 defaults change,
    re-run \texttt{test\_rmsc04.py}, \texttt{baselines.py}, and (if applicable)
    the six-policy runner so JSON and figures agree; update
    Tables~\ref{tab:rmsc04-gym-summary} and~\ref{tab:native-sim-summary} in this
    note.

    \item \textbf{Tie RL training to Section~\ref{sec:design-directions}.}
    Use domain randomization over background parameters, richer observations
    than the raw Gym tensor alone, and evaluation against both Gym heuristics
    and optional native sanity checks so $\pi(o_t)$ and $\pi(o_t,b_t)$ are not
    overfit to a single evaluation story.
\end{itemize}

In short, I will keep iterating on this by turning the current broad sweep into
a smaller, targeted regime-identification experiment. The goal is to produce
clearer market regimes first, then train the belief-aware RL agent to infer
those regimes from public observations.
