...
The default AAPL-calibrated \texttt{rmsc04} regime rewards short-horizon
momentum more than mean reversion. This matters for belief-aware RL training:
if the goal is to test whether an agent can infer and exploit fundamental
anchoring, the environment should likely be shifted toward stronger value-agent
activity, for example by increasing \texttt{num\_value\_agents} or
\texttt{lambda\_a}. If the goal is to learn robust trading under the current
default market, \texttt{MOMENTUM} is the benchmark to beat, not \texttt{MR}.

The sweep figures are useful for selecting candidate regimes, but most
parameter effects need more episodes or the raw sweep table before they should
be treated as statistically reliable.
