...
\noindent\textbf{TODO:} Define exact thresholds for the oracle heuristic. Use the crossover point
from the \texttt{num\_value\_agents} sweep (Figure~8, v1 report) as the anchor, and determine
momentum/noise boundaries empirically from baseline sweep data.
...
Three experimental runs, in order:
...
\item \textbf{Oracle.} $b_t = b_t^{\text{oracle}}$ (ground truth one-hot regime label). Policy
...
estimating the regime rather than knowing it.
...
\bottomrule
\end{tabular}
\caption{Evaluation metrics across all three runs.}
\end{table}
