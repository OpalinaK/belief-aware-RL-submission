"""
train_ppo_oracle_reward.py — E2: PPO with oracle regime belief in reward (reward shaping).

Trains PPO on the standard 9-dim oracle observation (same as E1) but adds an alignment
bonus to the reward during training:
  shaped_reward = env_reward + alpha * align(action, regime)

The reward shaping wrapper is stripped at eval time — eval uses RegimeAdapter only.
This tests whether reward shaping alone can improve performance vs the v4 baseline.

Outputs:
  results/e2_belief_reward/seed_{seed}/eval_metrics.json
  results/e2_belief_reward/seed_{seed}/eval_episodes.json
  results/e2_belief_reward/seed_{seed}/train_metrics.json
  results/e2_belief_reward/seed_{seed}/timing.json
  results/e2_belief_reward/seed_{seed}/checkpoints/

Usage:
  .venv/bin/python abides-gym/scripts/train_ppo_oracle_reward.py --seed 0
...
def parse_args():
    ...
    parser = argparse.ArgumentParser(description="E2: Train PPO with oracle regime belief in reward")
    ...
    parser.add_argument("--alpha",               type=float, default=0.001,
                        help="Reward shaping coefficient (from E2a sweep)")
    return parser.parse_args()


def main():
    args = parse_args()
    out_dir = Path(args.out_dir or f"results/e2_belief_reward/seed_{args.seed}")
    checkpoints_dir = out_dir / "checkpoints"
    out_dir.mkdir(parents=True, exist_ok=True)
    checkpoints_dir.mkdir(parents=True, exist_ok=True)

    _ts_log(
        f"E2 starting: out_dir={out_dir} timesteps={args.timesteps} "
        f"timestep={args.timestep} seed={args.seed} workers={args.num_workers} alpha={args.alpha}"
    )

    train_env_config = build_env_config(args, debug_mode=args.train_debug_mode, info_mode=args.train_info_mode)
    train_env_config["reward_alpha"] = float(args.alpha)
